...
def connected_components_with_gradients(
    ...
    if spatial_dims == 3:
        B, C, H, W, Z = image.shape

        # Precompute a mask with the valid values
        mask = image >= threshold
        image = torch.ceil(image * mask)

        out = torch.arange(B * H * W * Z, device=image.device, dtype=image.dtype).view(B, 1, H, W, Z)
        out = torch.where(mask, out, torch.tensor(0, device=image.device, dtype=image.dtype))

        # Use in-place operations to potentially speed up the loop
        for _ in range(num_iterations):
        ...
        out = torch.arange(B * H * W, device=image.device, dtype=image.dtype).view(B, 1, H, W)
        out = torch.where(mask, out, torch.tensor(0, device=image.device, dtype=image.dtype))
...
## BASED ON: https://github.com/pytorch/vision/blob/a192c95e77a4a4de3a8aeee45130ddc4d2773a83/torchvision/ops/boxes.py
def distance_box_iou_2D(center_output, min_output, max_output, center_label, min_label, max_label, smoother=1e-7):
    ## Calculate IoU based on the boxes information
    volume_output = (max_output[:,0] - min_output[:,0]) \
        * (max_output[:,1] - min_output[:,1])
    volume_label = (max_label[:,0] - min_label[:,0]) \
    ...
    centers_distance_squared = (
        (x_p[:, None] - x_g[None, :]) ** 2) + ((y_p[:, None] - y_g[None, :]) ** 2)
    
    lti = torch.min(min_output[:, None, :], min_label[:, :])  # [N,M,3]
    rbi = torch.max(max_output[:, None, :], max_label[:, :])  # [N,M,3]
    whi = (rbi - lti).clamp(min=0)  # [N,M,3]
    diagonal_distance_squared = (whi[:, :, 0] ** 2) + (whi[:, :, 1] ** 2) + smoother
...
####
def msge_loss_2D(true, pred, focus, focus_shape):
...
####
def msge_loss_3D(true, pred, focus, focus_shape):
    """Calculate the mean squared error of the gradients of 
    horizontal and vertical map predictions. Assumes 
    channel 0 is Vertical and channel 1 is Horizontal.
    Args:
    ...
    def get_sobel_kernel(size):
        ...
        h, v, z = torch.meshgrid(h_range, v_range, z_range, indexing="ij")
        kernel_h = h / (h * h + v * v + z * z + 1.0e-15)
        kernel_v = v / (h * h + v * v + z * z + 1.0e-15)
        kernel_z = z / (h * h + v * v + z * z + 1.0e-15)
        return kernel_h, kernel_v, kernel_z

    ####
    def get_gradient_hvz(hvz):
        """For calculating gradient."""
        kernel_h, kernel_v, kernel_z = get_sobel_kernel(5)
        kernel_h = kernel_h.view(1, 1, 5, 5, 5)  # constant
        kernel_v = kernel_v.view(1, 1, 5, 5, 5)  # constant
        kernel_z = kernel_z.view(1, 1, 5, 5, 5)  # constant

        h_ch = hvz[..., 0]  # Nx1xHxWxZ
        v_ch = hvz[..., 1]  # Nx1xHxWxZ
        z_ch = hvz[..., 2]  # Nx1xHxWxZ

        # can only apply in NCHW mode
        h_dh_ch = F.conv3d(h_ch, kernel_h, padding=2)
        v_dv_ch = F.conv3d(v_ch, kernel_v, padding=2)
        z_dz_ch = F.conv3d(z_ch, kernel_z, padding=2)
        dhvz = torch.stack((h_dh_ch, v_dv_ch, z_dz_ch), dim=-1)

        # nim = nib.Nifti1Image(h_dh_ch.squeeze().cpu().detach().numpy().astype('float32'), np.eye(4)) # nii_img.affine)
        # nib.save(nim, 'blobs/h_dh_ch.nii.gz')
        # nim = nib.Nifti1Image(v_dv_ch.squeeze().cpu().detach().numpy().astype('float32'), np.eye(4)) # nii_img.affine)
        # nib.save(nim, 'blobs/v_dv_ch.nii.gz')
        # nim = nib.Nifti1Image(z_dz_ch.squeeze().cpu().detach().numpy().astype('float32'), np.eye(4)) # nii_img.affine)
        # nib.save(nim, 'blobs/z_dz_ch.nii.gz')
        
        return dhvz

    focus = torch.stack([focus, focus, focus], axis=-1).float()
    true_grad = get_gradient_hvz(true)
    pred_grad = get_gradient_hvz(pred)
    loss = pred_grad - true_grad
...
def HoVerRad3D(mask, background_xyz=0, background_radial=0, smoother=1e-7):
    '''
    mask ~ [1, 1, H, W, D]
    returns:
    ...
    coords_x = 2 * (coords_x - x_min) / ((x_max-x_min) + smoother) - 1
    coords_y = 2 * (coords_y - y_min) / ((y_max-y_min) + smoother) - 1
    coords_z = 2 * (coords_z - z_min) / ((z_max-z_min) + smoother) - 1

    radial = torch.sqrt(coords_x*coords_x + coords_y*coords_y + coords_z*coords_z) + smoother
    mean_radial = radial[edge[0,0,...]>0.5].mean()
    # print("mean_radial: ", mean_radial)
    radial = radial / (mean_radial + smoother)
...
def HoVerRad2D(mask, background_xyz=0, background_radial=0, smoother=1e-7):
    '''
    mask ~ [1, 1, H, W]
    returns:
    coords_x, coords_y, radial ~ [1, 1, H, W]
    '''
    dims = (mask.shape[2], mask.shape[3])
    ...
    radial = torch.sqrt(coords_x*coords_x + coords_y*coords_y)
    thres = torch.nn.Sequential(torch.nn.Threshold(-0.5, 1), torch.nn.Threshold(0.5, 0))

    mask_threshold = thres(-mask).float()
    edge = F.max_pool2d(mask_threshold, 3, stride=1, padding=1) - mask_threshold
    ...
    radial = torch.sqrt(coords_x*coords_x + coords_y*coords_y) + smoother
    mean_radial = radial[edge[0,0,...]>0.5].mean()
    # print("mean_radial: ", mean_radial)
    radial = radial / (mean_radial + smoother)
    radial[mask_threshold[0,0,...]<0.5] = background_radial
    ...
    return coords_x, coords_y, radial

    
def HoVer3D(mask, background_xyz=0, smoother=1e-7):
    '''
    mask ~ [1, 1, H, W, D]
    returns:
    coords_x, coords_y, coords_z ~ [1, 1, H, W, D]
    '''
    dims = (mask.shape[2], mask.shape[3], mask.shape[4])
...
def HoVer2D(mask, background_xyz=0, smoother=1e-7):
    '''
    mask ~ [1, 1, H, W]
    returns:
    coords_x, coords_y ~ [1, 1, H, W]
    '''
    dims = (mask.shape[2], mask.shape[3])
